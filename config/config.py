...
# Словарь с количеством запусков каждой программы из llvm-test-suite во время обучения.
# Костыль для того, чтобы не запускать часто программы, которые исполняются очень долго,
# для замера времени
RUNS_NUMBER = {
...
# Замеры времени исполнения на устройстве, используемом для обучения.
# Костыль для экономии времени обучения
O3_RUNTIMES = {
